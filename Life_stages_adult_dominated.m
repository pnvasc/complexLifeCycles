...
function Life_stages_adult_dominated
...
dname = ['AD_zj' num2str(zj) '_za' num2str(za) '_' date];% '_asymm'];
...
for iter = 1:3
    ...
    scatter(xJ, xA, sqrt(CA/sum(CA))*2e2, 'MarkerEdgeColor', 'k')
        hold on;
        xlabel('x_J')
        ylabel('x_A')
        axis([0 1 0 1])
...
%% NESTED FUNCTIONS
    ...
    function euler
        ...
        dCJ_dt = inf;
        dCA_dt = inf;
        max_abs_change = 1e-5; % maximum change size for Euler's forward method
        tic
    ...
    function plotting
        figure(1)
        scatter(xJ, xA, sqrt(CA/sum(CA))*2e2, 'MarkerEdgeColor', col(ceil(t/time*colres),:))
        hold on;
        xlabel('x_J')
        ylabel('x_A')
        axis([0 1 0 1])
        title({['Adult dominated. Parameters:']
            ['zj = ', num2str(zj), ', za = ', num2str(za), ...
            %', initial \Theta_J = ', num2str(xJ_in), ', initial \Theta_A = ', num2str(xA_in), 
            %', generations = ', num2str(time) %, ', rJ = ', num2str(rJ), ', KJ = ' num2str(KJ)
            ]})
